import React from "react";
import "./style.css";

export const Celularacademy = () => {
  return (
    <div className="celularacademy">
      <div className="div">
        <div className="overlap">
          <p className="minhas-notas-fiscais">
            Minhas notas fiscais internacionais, em 2015.&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Mais de 8 anos atrás.
          </p>
          <p className="profiss-o-tradutor">
            <span className="text-wrapper">Profissão </span>
            <span className="span">Tradutor</span>
            <span className="text-wrapper"> - do Zero ao Sucesso</span>
          </p>
          <div className="text-wrapper-2">Metodo</div>
          <p className="qualquer-pessoa-com">
            <span className="text-wrapper">Qualquer pessoa com um </span>
            <span className="span">segundo idioma</span>
            <span className="text-wrapper"> pode se tornar tradutor.</span>
          </p>
          <div className="overlap-group">
            <div className="rectangle" />
            <p className="imers-o-dias-para">
              Imersão 3 dias para entrar no Mercado <br />
              25 - 26 de novembro&nbsp;&nbsp;e 9 de dezembro!
            </p>
          </div>
          <p className="p">Vem comigo que eu te ensino como usar seu idioma para ganhar em dolar fora do brasil</p>
          <div className="whatsapp-video" />
          <p className="saia-do-ZERO-e-torne">
            Saia do ZERO e torne-se um tradutor de sucesso
            <br />
            Como começar
            <br />
            Como conseguir clientes
            <br />
            Como ganhar experiência comprovada
            <br />
            Como se apresentar&nbsp;&nbsp;para o mercado
            <br />
            Tecnologias obrigatórias para ingressar no mercado
          </p>
          <a className="link" href="https://mpago.la/2vM3EcD" rel="noopener noreferrer" target="_blank">
            <p className="text-wrapper-3">Clique e destrave seu futuro</p>
          </a>
          <div className="text-wrapper-4">Checa os valores:</div>
          <div className="text-wrapper-5">Dá uma olhada</div>
          <img className="image" alt="Image" src="/img/image-3-1.png" />
          <img className="img" alt="Image" src="/img/image-2-1.png" />
          <p className="curso-com-caysa-biel">
            <span className="text-wrapper-6">Curso com Caysa Biel </span>
            <span className="text-wrapper-7">&nbsp;&nbsp;-</span>
            <a href="https://www.instagram.com/caysabiel/" rel="noopener noreferrer" target="_blank">
              <span className="text-wrapper-8">&nbsp;</span>
            </a>
            <a href="https://www.instagram.com/caysabiel/" rel="noopener noreferrer" target="_blank">
              <span className="text-wrapper-9">@caysabiel</span>
            </a>
          </p>
          <div className="empresa-de-traduo" />
        </div>
        <p className="text-wrapper-10">Notas fiscais de tradutores da minha empresa</p>
        <img className="PO" alt="Po" src="/img/po-335-3.png" />
        <img className="PO-2" alt="Po" src="/img/po-335-34-1.png" />
        <img className="PO-3" alt="Po" src="/img/po-565-1-1.png" />
        <img className="PO-4" alt="Po" src="/img/po-580-1-1.png" />
        <div className="text-wrapper-11">E quem sou eu?</div>
        <div className="imagens">
          <div className="overlap-group-2">
            <div className="div-absolute" />
            <div className="frame">
              <div className="servio-de-trado-wrapper">
                <img
                  className="servio-de-trado"
                  alt="Servio de trado"
                  src="/img/servi-o-de-trad-o-profissional-1.png"
                />
              </div>
            </div>
            <div className="empresa-de-traduo-2" />
            <div className="empresa-de-traduo-3" />
          </div>
        </div>
        <div className="overlap-2">
          <div className="flexcontainer">
            <p className="text">
              <span className="text-wrapper-12">
                Me chamo Caysa Biel e cresci no meio do mato, nem eletricidade tinha.
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-12">
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-12">
                {" "}
                Aos 13 anos busquei meu sonho no Rio de Janeiro. Ser jogador de futebol.
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-12">
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-12">
                Alcancei esse sonho, na Alemanha. 5 anos pelos campos de lá.
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-12">
                Estudei jornalismo na Espanha em 2006.
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-12">
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-12">
                Em 2012 voltei pro Brasil, para abrir a filial de uma empresa alemã, de cursos de idiomas online.
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-12">
                Dois anos na empresa e disse CHEGA! Rompi com o CLT!
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-12">
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-12">
                Minha felicidade não estava ali.
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-12">
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-12">
                Ser dono do meu tempo e ter meus próprios projetos é inegociável na minha vida.
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-12">
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-12">
                Após 1 ano como jornalista e 70 reportagens para uma TV iraniana.
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-13">
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-14">
                Me torno TRADUTOR
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-14">
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-15">
                Depois disso, tudo mudou...
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-15">
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-15">
                Obrigado irmão e mentor Turian. Me colocou rapidamente no mercado.
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-15">
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-15">
                Em 2015 fundei a Magma Translation.
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-15">
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-15">
                Em 9 anos no Mercado de Tradução atuei com sucesso como tradutor e consolidei minha empresa de tradução,
                no mercado nacional e internacional.
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-15">
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-15">
                Muitos tropeços e muitos acertos nessa jornada.
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-15">
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-15">
                Agora coloco todo esse conhecimento à sua disposição.
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-15">
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-15">
                Para VOCÊ ingressar no mercado de tradução e<br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-15">
                PULAR DO INÍCIO DIRETO PRO SUCESSO!
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-16">
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-16">
                <br />
              </span>
            </p>
            <p className="text">
              <span className="text-wrapper-16">{""}</span>
            </p>
          </div>
          <img className="line" alt="Line" src="/img/line-1.svg" />
        </div>
        <div className="overlap-3">
          <div className="overlap-4">
            <div className="overlap-5">
              <div className="rectangle-2" />
              <p className="text-wrapper-17">CVE- Círculo Virtuoso da Experiência – Sua entrada no Mercado</p>
              <div className="flexcontainer-2">
                <p className="span-wrapper">
                  <span className="text-wrapper-18">
                    Trabalho voluntário, plataformas de freela, prática e experiência na Magma Translation.
                    <br />
                  </span>
                </p>
                <p className="span-wrapper">
                  <span className="text-wrapper-18">
                    Após o curso terá experiência no currículo para entrar no Mercado de Tradução.
                  </span>
                </p>
              </div>
              <div className="group">
                <div className="overlap-6">
                  <p className="text-wrapper-19">Prospecção Infinita - Sua chave pro Sucesso.</p>
                  <div className="flexcontainer-3">
                    <p className="text-2">
                      <span className="text-wrapper-18">
                        Cliente nacionais e internacionais.
                        <br />
                      </span>
                    </p>
                    <p className="text-2">
                      <span className="text-wrapper-18">
                        Aprenda como ganhar clientes e cobrar em Real, Dólar e EUR.
                      </span>
                    </p>
                  </div>
                </div>
              </div>
              <div className="overlap-wrapper">
                <div className="overlap-7">
                  <div className="text-wrapper-20">Portfólio Vitalício</div>
                  <div className="flexcontainer-4">
                    <p className="text-2">
                      <span className="text-wrapper-18">
                        Como manter seus clientes em parcerias de longo prazo.
                        <br />
                      </span>
                    </p>
                    <p className="text-2">
                      <span className="text-wrapper-18">Ações e comportamentos que mantém portfólio.</span>
                    </p>
                  </div>
                </div>
              </div>
              <div className="overlap-group-wrapper">
                <div className="overlap-8">
                  <p className="text-wrapper-21">Marketing da Sedução – Como se apresentar para seu Recrutador</p>
                  <div className="flexcontainer-5">
                    <p className="text-2">
                      <span className="text-wrapper-18">
                        E-mail, currículo, redes sociais e plataforma de Jobs.
                        <br />
                      </span>
                    </p>
                    <p className="text-2">
                      <span className="text-wrapper-18">
                        Tudo que você precisa saber para impressionar seu recrutador.
                      </span>
                    </p>
                  </div>
                </div>
              </div>
              <div className="div-wrapper">
                <div className="overlap-9">
                  <div className="text-wrapper-22">CLE: Crescimento Linear Estruturado</div>
                  <div className="flexcontainer-6">
                    <p className="text-2">
                      <span className="text-wrapper-18">
                        Passo a passo para alcançar novos clientes e crescer portfólio.
                        <br />
                      </span>
                    </p>
                    <p className="text-2">
                      <span className="text-wrapper-18">
                        Método de prospecção desenvolvido em 10 anos de Mercado de Tradução.
                      </span>
                    </p>
                  </div>
                </div>
              </div>
            </div>
            <div className="group-2">
              <div className="overlap-10">
                <p className="text-wrapper-23">Cat Tool – Sem elas você não é tradutor.</p>
                <div className="flexcontainer-7">
                  <p className="text-2">
                    <span className="text-wrapper-18">
                      Teoria e prática das ferramentas de tradução.
                      <br />
                    </span>
                  </p>
                  <p className="text-2">
                    <span className="text-wrapper-18">Todo projeto de tradução é feito dentro dessas ferramentas.</span>
                  </p>
                </div>
              </div>
            </div>
          </div>
          <div className="overlap-11">
            <div className="group-3">
              <div className="overlap-12">
                <div className="text-wrapper-24">Partindo para a prática</div>
                <p className="text-wrapper-25">Primeiro projeto. O início de sua experiência.</p>
              </div>
            </div>
            <div className="overlap-13">
              <div className="group-4">
                <div className="overlap-14">
                  <p className="text-wrapper-26">Precificação – Quanto cobrar por seu trabalho</p>
                </div>
              </div>
              <div className="flexcontainer-8">
                <p className="span-wrapper">
                  <span className="text-wrapper-18">
                    Os preços do mercado e o que vai cobrar por seu trabalho.
                    <br />
                  </span>
                </p>
                <p className="span-wrapper">
                  <span className="text-wrapper-18">
                    Te entregamos o OURO. Sua estratégia para se colocar no Mercado de tradução.
                  </span>
                </p>
              </div>
              <div className="group-5">
                <div className="overlap-15">
                  <p className="text-wrapper-27">Mercado de Tradução - Informação é poder</p>
                  <div className="flexcontainer-9">
                    <p className="text-2">
                      <span className="text-wrapper-18">
                        Como funciona, clientes, requisitos e certificados.
                        <br />
                      </span>
                    </p>
                    <p className="text-2">
                      <span className="text-wrapper-18">
                        10 anos de conhecimento para você.
                        <br />
                      </span>
                    </p>
                    <p className="text-2">
                      <span className="text-wrapper-18">
                        Aprenda a se posicionar no mercado e saia na frente dos seus concorrentes.
                      </span>
                    </p>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="group-6">
          <div className="group-7">
            <p className="text-wrapper-28">Magma Translation | Todos os direitos reservados. ©Magma™</p>
          </div>
          <img className="magma-translation" alt="Magma translation" src="/img/magma-translation-png.png" />
        </div>
        <div className="text-wrapper-29">Módulos do Curso</div>
        <div className="overlap-16">
          <div className="overlap-17">
            <div className="group-8">
              <div className="overlap-18">
                <div className="rectangle-3" />
                <div className="text-wrapper-30">Bônus:</div>
                <div className="flexcontainer-10">
                  <p className="text-2">
                    <span className="text-wrapper-18">
                      5 primeiros que comprarem o curso ganham 2 projetos de 500 palavras revisados pela equipe da Magma
                      Translation.
                      <br />
                    </span>
                  </p>
                  <p className="text-2">
                    <span className="text-wrapper-18">Projetos simulados de tradução da Magma Translation.</span>
                  </p>
                </div>
              </div>
            </div>
            <div className="group-9">
              <div className="overlap-19">
                <div className="text-wrapper-31">Super Bônus:</div>
                <div className="flexcontainer-11">
                  <p className="text-2">
                    <span className="text-wrapper-18">
                      10 e-mails de Gerentes de Projetos por dia durante 3 meses para envio de currículo.
                      <br />
                    </span>
                  </p>
                  <p className="text-2">
                    <span className="text-wrapper-18">
                      ATENÇÃO: SÃO emails diretos de Gerentes de Projetos. NÃO são emails gerais de empresa.
                    </span>
                  </p>
                </div>
              </div>
            </div>
          </div>
          <div className="overlap-20">
            <div className="group-10">
              <div className="overlap-21">
                <p className="text-wrapper-32">Bônus Exclusivo curso Profissão Tradutor – do Zero ao Sucesso!</p>
                <div className="flexcontainer-12">
                  <p className="text-3">
                    <span className="text-wrapper-33">
                      Experiência Magma Translation.
                      <br />
                    </span>
                  </p>
                  <p className="text-3">
                    <span className="text-wrapper-34">
                      <br />
                    </span>
                  </p>
                  <p className="text-3">
                    <span className="text-wrapper-35">
                      Seu currículo com experiência profissional de uma <br />
                    </span>
                  </p>
                  <p className="text-3">
                    <span className="text-wrapper-35">
                      empresa consolidada no mercado nacional e internacional.
                      <br />
                    </span>
                  </p>
                  <p className="text-3">
                    <span className="text-wrapper-35">
                      <br />
                    </span>
                  </p>
                  <p className="text-3">
                    <span className="text-wrapper-35">
                      <br />
                    </span>
                  </p>
                  <p className="text-3">
                    <span className="text-wrapper-35">
                      <br />
                    </span>
                  </p>
                  <p className="text-3">
                    <span className="text-wrapper-35">
                      <br />
                    </span>
                  </p>
                  <p className="text-3">
                    <span className="text-wrapper-35">{""}</span>
                  </p>
                </div>
              </div>
            </div>
            <div className="flexcontainer-wrapper">
              <div className="flexcontainer-13">
                <p className="text-4">
                  <span className="text-wrapper-36">
                    <br />
                  </span>
                </p>
                <p className="text-4">
                  <span className="text-wrapper-37">
                    Avaliação positiva Magma Translation nas Plataformas de Job.
                    <br />
                  </span>
                </p>
                <p className="text-4">
                  <span className="text-wrapper-37">
                    <br />
                  </span>
                </p>
                <p className="text-4">
                  <span className="text-wrapper-36">
                    Você escolhe seu hotel e restaurante pela avaliação dos usuários.
                    <br />
                  </span>
                </p>
                <p className="text-4">
                  <span className="text-wrapper-36">Seu recrutador escolhe você pelas avaliações nas plataformas.</span>
                </p>
              </div>
            </div>
            <a className="link-2" href="https://mpago.la/2vM3EcD" rel="noopener noreferrer" target="_blank">
              <p className="text-wrapper-38">Clique e destrave seu futuro</p>
            </a>
          </div>
        </div>
        <div className="text-wrapper-39">Bônus</div>
        <div className="group-11">
          <div className="overlap-22">
            <div className="text-wrapper-40">Revisão, Perguntas e Respostas</div>
            <div className="flexcontainer-14">
              <p className="text-2">
                <span className="text-wrapper-18">
                  Revisão de Currículos
                  <br />
                </span>
              </p>
              <p className="text-2">
                <span className="text-wrapper-18">
                  Revisão de E-mail
                  <br />
                </span>
              </p>
              <p className="text-2">
                <span className="text-wrapper-18">
                  Revisão Plataforma de Jobs
                  <br />
                </span>
              </p>
              <p className="text-2">
                <span className="text-wrapper-18">
                  Entrega revisão 5 premiados
                  <br />
                </span>
              </p>
              <p className="text-2">
                <span className="text-wrapper-18">Perguntas e Respostas</span>
              </p>
            </div>
          </div>
        </div>
        <div className="group-12">
          <div className="overlap-23">
            <div className="rectangle-4" />
            <p className="text-wrapper-41">Aula extra: Como evitar golpes</p>
            <p className="text-wrapper-42">Aula extra: Como usar a plataforma de E-mail Marketing.</p>
          </div>
        </div>
        <div className="group-13">
          <p className="text-wrapper-43">As Profissões dentro da Profissão</p>
          <p className="tradu-o-t-cnica">
            Tradução técnica, Localização, audiovisual, MTPE (Machine Translation Post Editing), Legendagem,
            Transcriação, Revisão, editorial e tradução juramentada.
            <br />
            Conheça todas as áreas que você pode atuar no Mercado de Tradução.
          </p>
        </div>
        <div className="overlap-24">
          <div className="group-14">
            <div className="overlap-25">
              <div className="text-wrapper-44">Tudo isso por apenas</div>
              <div className="overlap-26">
                <div className="overlap-27">
                  <div className="text-wrapper-45">997,00 R$</div>
                  <div className="overlap-28">
                    <div className="text-wrapper-46">1997,00 R$</div>
                    <p className="text-wrapper-47">Valor para as próximas turmas</p>
                    <div className="group-15">
                      <div className="overlap-group-3">
                        <img className="line-2" alt="Line" src="/img/line-4.svg" />
                        <img className="line-3" alt="Line" src="/img/line-3.svg" />
                      </div>
                    </div>
                  </div>
                </div>
                <p className="valor-para-essa"> Valor para essa primeira turma</p>
              </div>
            </div>
          </div>
          <a className="link-3" href="https://mpago.la/2vM3EcD" rel="noopener noreferrer" target="_blank">
            <p className="text-wrapper-48">Clique e destrave seu futuro</p>
          </a>
        </div>
      </div>
    </div>
  );
};
